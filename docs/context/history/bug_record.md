...
## BUG-013: 3个模型缺少对应视图（test_03 发现、Sprint9 修复）

**发现时间**: 2026-07-23
**发现场景**: test_runner.py 首次执行 → test_03_action_res_model_has_view 失败
**根因文件**: container_service_views.xml / surcharge_views.xml / transport_fee_views.xml
**严重等级**: LEVEL2 — 菜单可打开但点记录无 form 视图会报错
**根因分类**: 设计遗漏（视图缺失）

### 错误现象
```python
Found 3 act_window(s) referencing views that do not exist for their model:
  Container Service Requests     model=container.service.request    view_mode=list,form
  Surcharge Types                model=tlmp.surcharge.type         view_mode=list,form
  Fee Lines                      model=transport.fee.line          view_mode=list,form
```

### 根因
act_window 的 view_mode 声明了 "list,form" 两种视图类型，但对应模型缺少 form 视图（
container.service.request 和 transport.fee.line），甚至完全没有视图（tlmp.surcharge.type）。

### 修复
- container_service_views.xml: 新增 form 视图
- surcharge_views.xml: 新增 list + form 视图
- transport_fee_views.xml: 新增 form 视图

### 落地验证
- test_runner.py: 4项 TestCase 全部通过 ✅
- verify.py: 8项门禁全部通过 ✅

...
class Picking(models.Model):
    ...
    def _put_in_pack(self, move_line_ids):
        # 判断是否为收货单据(receipt)
        if self.picking_type_code == 'incoming':  # [2,7](@ref)
            # 从上下文获取托盘条码，支持多种来源
            pallet_barcode = self.env.context.get('pallet_barcode') or \
                             self.env.context.get('context', {}).get('pallet_barcode')
            if not pallet_barcode:
                raise UserError(_("Please scan the pallet barcode before proceeding."))

            # Check if a package with the same name already exists
            package = self.env['stock.quant.package'].search([('name', '=', pallet_barcode)], limit=1)
            if not package:
                # Create a new package if it doesn't exist
                package = self.env['stock.quant.package'].create({'name': pallet_barcode})

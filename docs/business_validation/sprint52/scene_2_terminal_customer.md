# Sprint52-B Scene 2: Terminal → Customer

> 契约：`INT-TMS-SPRINT52B-001`
> 场景码：`terminal_to_customer`
> 链路：request → inquiry → quote → transport.order → delivery

## 前置条件

- `common_pre_check.md` 已执行。
- Master Data：Terminal1（起点）、Customer（终点，可复用现有客户或自由地址）、Carrier 可用。

## 验证步骤

| 步骤 | 操作 | 预期 |
| :--- | :--- | :--- |
| 1 | 创建 request，场景 = terminal_to_customer | state=draft，scene/起终点/地址正确 |

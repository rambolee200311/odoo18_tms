...
| Inquiry | 1071 | selected（FIX-003 迁移） |
...
| Module Version | 1.0.124 → 1.0.125 → 1.0.126 |

## Sprint52-Fix3 状态语义迁移（wd_tlms 1.0.126，2026-08-10）

按 `INT-TMS-SPRINT52FIX-003`：
- inquiry `accepted` 统一迁移为 `selected`（24 条），
...
- 验证：XML-RPC 升级 1.0.126 PASS，日志零 ERROR，
  S2 新链回归 PASS（quote/order fee 480/380，order closed）。

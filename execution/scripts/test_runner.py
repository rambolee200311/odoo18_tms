...
def _mark_module_upgrade():
    """Try to mark wd_tlms as 'to upgrade' in database to force re-read."""
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute("UPDATE ir_module_module SET state='to upgrade' WHERE name='wd_tlms'")
        affected = cur.rowcount
        conn.commit()
        cur.close()
        conn.close()
        if affected:
            print(f"  DB: wd_tlms marked to upgrade ({affected} row)")
        else:
            print("  DB: wd_tlms not found in ir_module_module")
        return True
    except ImportError:

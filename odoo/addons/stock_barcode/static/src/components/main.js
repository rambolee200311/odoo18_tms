...
class MainComponent extends Component {
    ...
    putInPack(ev) {
        ...
        // 获取服务（确保在 setup 中已初始化）
        const notification = this.notification || {
            add: (msg, options) => console.log("Notification:", msg, options)
        };
        ...
        // 创建对话框关闭后的处理函数
        const onResultHandler = async (barcode) => {
            if (!barcode) {
                notification.add(_t("No barcode detected"), {
                    type: "warning",
                    duration: 3000
                });
                return;
            ...
            try {
                ...
                if (navigator.clipboard?.writeText) {
                    ...
                    notification.add(_t("Barcode copied to clipboard"), {
                        type: "success",
                        duration: 2000
                    });
                }
                ...
                // 执行核心业务逻辑
                await model._putInPack({
                    pallet_barcode: barcode
                });

                notification.add(_t("Operation successful"), {
                    type: "success",
                    duration: 3000
                });
            } catch (error) {
                console.error("PutInPack error:", error);
                ...
                notification.add(_t("Error: ") + errorMsg, {
                    type: "danger",
                    duration: 5000
                });
            }
        ...
        if (!dialogService || !dialogService.add) {
            ...
            notification.add(_t("Cannot open scanner dialog"), {
                type: "danger"
            });
            return;
        }
        ...
        // 打开对话框并添加关闭后处理
        const dialogRef = dialogService.add(ManualBarcodeScanner, {
            onResult: onResultHandler.bind(this),
            onError: (error) => {
                notification.add(_t("Scanner error: ") + error.message, {
                    type: "danger"
                });
            }
        });
        ...
        dialogRef.onClose = () => {
            if (actionService && actionService.doAction) {
                ...
                console.log("Dialog closed, returning to previous view");
                // 回退方案：尝试关闭当前视图
                if (this.env && this.env.closeView) {
                    this.env.closeView();
                }
            }
        };
